...
# cap = cv.VideoCapture("video_data/lunge/good_lunge/good_lunge_0.mp4")

frame_idx = 0
n_reps = 0

MIN_ANGLE = 90
UPRIGHT_POS_ANGLE = 140
    ...
    while True:
        ...
        if wait_over:
            if not initial and not low and not back_up:
                if right_angle > UPRIGHT_POS_ANGLE and left_angle > UPRIGHT_POS_ANGLE:
                    initial = True

            elif initial and not low:
                if right_angle < MIN_ANGLE and left_angle < MIN_ANGLE:
                    low = True

...
# bad rep means we use that as feedback add .5 
...
class TimedFeedback:
    ...
    def __init__(self):
        ...
        self.score_map = {
            ...
            "good_lunge": 1.0,
            "low_hip_pushup": 0.25,
            "high_hip_pushup": 0.50,
            "partial_lunge": 0.25,
            "angled_back_lunge": 0.50,
        }

        self.rep_map = {
            "pushup": (90, 150), # final
            "squat": (87.5, 150), # final
            "lunge": (90, 140), # final
        }
    ...
    def run_feedback_model(self, buffer):
    ...
    def draw_overlay(self, frame, on_squat=False):
        h, w = frame.shape[:2]
        ...
        if not on_squat:
            ...
            cv.putText(
                ...
                (15, 90), cv.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2, cv.LINE_AA
            )
        return frame
    
    def run_on_video_squat_version(self, path, show_frame: bool) -> str:
        ...
        self.n_reps = 0
        frame_idx = 0
        self.exercise_utils = Exercise(self.exercise, model=None, on_colab=False)
        ...
        with self.PoseLandmarker.create_from_options(self.options) as self.landmarker:
            while True:
                ...
                if show_frame:
                    self.draw_overlay(frame, on_squat=True)
                    cv.imshow("frame", frame)
                    if cv.waitKey(1) & 0xFF == ord('q'):
                        break
    ...
    def run_on_video(self, path, exercise: str, show_frame: bool) -> str:
        ...
        if exercise == "squat":
            out = self.run_on_video_squat_version(path=path, show_frame=show_frame) # redirect
            return out
